...
module Mail #:nodoc:
  ...
  class MadMimi
    ...
    def setup_options_from_mail_and_get_mad_mimi_body(mail)
      ...
      mad_mimi_options = mad_mimi_options_as_hash(mail[:mad_mimi]) # Convert mad_mimi Mail::Field back to a Hash of options
      ...
      self.settings[:promotion_name] = mad_mimi_options.delete(:promotion_name) || (mail.respond_to?(:mailer_action) ? mail.mailer_action : nil)
      mad_mimi_options
            
    end
    
    def mad_mimi_options_as_hash(field)
